                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-xl-2">
                                    ...
                                    <button type="button" id="license-btn"
                                        ...
                                        <?php 
                                            if($startYear === $shopData['RenewalDate']){ ?>
                                        Renew License
                                        <?php } else { ?>
                                        Apply for License <?php } ?></button>
                                </div>
                            </div>
                ...
                if (data.length > 0) {
                    ...
                    $.each(data, function(index, item) {
                        ...
                        if (item.PaymentStatus.toLowerCase() === "success") {
                           ...
                           actionBtn =`<button class="btn btn-sm btn-success" onclick="paymentGateway(${item.Billing_Cd}, ${item.BillAmount}, ${shopCd})">Pay Now</button>`;
                        }
                        ...
                        tableHtml += `<tr class="bill-row text-center" data-index="${index}">
                                                    ...
                                                    <td> 
                                                        <div style="display: flex; align-items: center;">
                                                            <a href="./action/bill_pdf.php?billing_id=${item.Billing_Cd}"   data-tooltip="Download the License Invoice" style="margin-right: 12px;" target="_blank">
                                                               <span class ="badge bg-warning" style="font-size: 14px;"> License Invoice </span>
                                                            </a>`;
                                if (item.PaymentStatus.toLowerCase() === "success") {                            
                                         tableHtml += ` <a href="./action/reciept.php?Transaction_Cd=${item.Transaction_Cd}" data-bs-toggle="tooltip" data-tooltip="Download the Payment Receipt" style="margin-right: 12px;" target="_blank">
                                                                <span class ="badge bg-info" style="font-size: 14px;"> Payment Receipt</span>
                                                            </a>`;
                                }
                        tableHtml += `              </div>
                                                    </td>
...
function applyforlicense(shopCd) {
    ...
    Swal.fire({
        html: `After you complete the payment, your license will be valid from <strong>${StartDate}</strong> to <strong>${EndDate}</strong>`,
        icon: 'warning',
...
function paymentGateway(Billing_id, Amount, shopCd) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            //     redirectPaymentGateway(data.data);
            // }else{
            ...
            if (data.status.tolowercase() === 'success') {
                ...
                Swal.fire({
                    ...
                    text: 'Your license has been generated successfully.',
                    icon: 'success',
                    confirmButtonText: 'OK',
                    confirmButtonColor: '#3085d6'
                });
            } else if (data.status.tolowercase() === 'exists') {
                Swal.fire({
...
//function redirectPaymentGateway(data) {
//
//    $.ajax({
//        type: "POST",
//        url: 'PHP-GetEPay/PG/GetePayInvoicePg.php',
//        data: data,
//        success: function(response) {
//           
//        },
