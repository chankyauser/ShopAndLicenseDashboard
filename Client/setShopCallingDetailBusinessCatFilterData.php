<?php
    ...
    $query = "SELECT
        ...
        AND CONVERT(VARCHAR, ShopMaster.SRAssignedDate ,120) BETWEEN '$fromDate' AND '$toDate'
    ...
    LEFT JOIN CallingCategoryMaster AS ccm ON (ShopMaster.Calling_Category_Cd = ccm.Calling_Category_Cd)
    WHERE ShopMaster.IsActive = 1 AND ISNULL(ShopMaster.ShopStatus,'') = 'Verified'
    AND CONVERT(VARCHAR, ShopMaster.SRAssignedDate ,120) BETWEEN '$fromDate' AND '$toDate'
    AND ccm.Calling_Type = 'Calling'
    $businessCatCondition
    $CallingCategoryCondition
    $nodeCondition
                            ...
                            <div class="row">
                                <div class="col-md-2 col-12">
                                    <div class="form-group">
                                        ...
                                        <input type='date' name="fromDate" value="<?php echo $fromDate;?>" class="form-control pickadate" style="height:35px;"/>
                                    </div>
                                </div>
                                <div class="col-md-2 col-12">
                                    <div class="form-group">
                                        <label for="toDate">To Date</label>
                                        <input type='date' name="toDate" value="<?php echo $toDate;?>" class="form-control pickadate" style="height:35px;"/>
                                    </div>
                                </div>

                                <div class="col-md-2 col-12">
                                    <div class="form-group">
                                        <label for="toDate">Ward Name</label>
                                <select class="select2 form-control" name="nodeCd1" style="height:35px;">
                                    <span class="fi-rs-apps"></span> <span class="et"><option value="All">All Ward </option></span>
                                        <?php 
                                            foreach ($dataNode as $key => $valueNode) {
                                        ?>
                                            <option value="<?php echo $valueNode["Node_Cd"]; ?>"
                                            <?php if($valueNode["Node_Cd"] == $nodeCd) { ?> selected <?php } ?>
                                ...
                                <div class="col-md-2 col-12" id="pocketSurveyListId">
                                   <?php include 'dropdown-calling-cat-cd.php'; ?>
                                </div>

                                <div class="col-md-2 col-12 text-right"   style="margin-top: 25px;">
                                     <div class="form-group">
                                        <label for="refesh" ></label>
                                        <button type="button" name="refesh" class="btn btn-primary" onclick="setShopCallingBusinessCategoriesFilter(1,'All')" style="padding: 8px 10px;">Refresh</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                ...
                <div class="pagination-area" style="float:right;">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-start">
                            ...
                            ?>  
                                <li class="page-item"><a class="page-link" onclick="setShopCallingBusinessCategoriesFilter(<?php echo ($loopStart - 1);  ?>, '<?php echo $businessCatCd; ?>')" ><i class="fi-rs-arrow-small-left"></i></a></li>
                            <?php } ?>
                            ...
                            <?php
                                for($i=$loopStart;$i<=$loopStop;$i++){ 
                                    ...
                                    ?>
                                    <li class="page-item <?php echo $activePageCondition; ?>"><a class="page-link" onclick="setShopCallingBusinessCategoriesFilter(<?php echo $i; ?>, '<?php echo $businessCatCd; ?>')" ><?php echo $i; ?></a></li>
                            <?php } ?>
                            <?php if($totalRecords > $loopStop){ ?> 
                                <li class="page-item"><a class="page-link"  onclick="setShopCallingBusinessCategoriesFilter(<?php echo ($loopStop + 1);  ?>, '<?php echo $businessCatCd; ?>')" ><i class="fi-rs-arrow-small-right"></i></a></li>
                            <?php }  ?>
