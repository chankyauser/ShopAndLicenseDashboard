<section id="chartjs-charts">
  <?php 

    $db1=new DbOperation();
    $userName=$_SESSION['SAL_UserName'];
    $appName=$_SESSION['SAL_AppName'];
    $electionName=$_SESSION['SAL_ElectionName'];
    $developmentMode=$_SESSION['SAL_DevelopmentMode'];

    $queryRevenueBusinessCategory = " SELECT SUM(td.Amount) as LicenseAmount,
    bcm.BusinessCatName
    FROM ShopMaster as sm
    INNER JOIN BusinessCategoryMaster as bcm ON bcm.BusinessCat_Cd = sm.BusinessCat_Cd
    INNER JOIN TransactionDetails as td ON td.Shop_Cd = sm.Shop_Cd
    WHERE sm.IsActive = 1
    AND bcm.IsActive = 1
    GROUP BY bcm.BusinessCatName
    ORDER BY BusinessCatName ;";

    $dataRevenueBusinessCategory = $db1->ExecutveQueryMultipleRowSALData($queryRevenueBusinessCategory, $electionName, $developmentMode);

    // $queryCallingNode = "SELECT nm.NodeName, COUNT(sm.Shop_Cd) as ShopCallingCount FROM ShopMaster as sm
    // INNER JOIN PocketMaster as pm ON pm.Pocket_Cd = sm.Pocket_Cd
    // INNER JOIN NodeMaster as nm ON nm.Node_Cd = pm.Node_Cd
    // WHERE 
	// sm.Calling_Category_Cd IS NOT NULL
    // AND sm.IsActive = 1
    // AND pm.IsActive = 1
    // AND nm.IsActive = 1
	// GROUP BY nm.NodeName ;";

    // $dataNodeWiseCallingShop = $db1->ExecutveQueryMultipleRowSALData($queryCallingNode, $electionName, $developmentMode);

    // $queryCallingWard = "SELECT nm.Ward_No, COUNT(sm.Shop_Cd) as ShopCallingCount FROM ShopMaster as sm
    // INNER JOIN PocketMaster as pm ON pm.Pocket_Cd = sm.Pocket_Cd
    // INNER JOIN NodeMaster as nm ON nm.Node_Cd = pm.Node_Cd
    // WHERE 
    // sm.Calling_Category_Cd IS NOT NULL
    // AND sm.IsActive = 1
    // AND pm.IsActive = 1
    // AND nm.IsActive = 1
	// GROUP BY nm.Ward_No ;";

    // $dataWardWiseCallingShop = $db1->ExecutveQueryMultipleRowSALData($queryCallingWard, $electionName, $developmentMode);

    ?>


    <div id="businessCategoryWiseSurveyDetailId">                               
        <?php include 'datatbl/tblGetShopRevenueSummary.php'; ?>
    </div> 

    </section>