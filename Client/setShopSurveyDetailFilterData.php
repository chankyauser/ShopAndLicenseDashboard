<?php
   ...
   if($nodeName == "All"){
    ...
    if($documentStatus == "All"){
        ...
        $documentStatusCondition = " AND ShopMaster.SurveyDate IS NOT NULL AND (ISNULL(ShopMaster.ShopOutsideImage1,'') <> '' OR ISNULL(ShopMaster.ShopOutsideImage2,'') <> '') AND ( ISNULL(ShopMaster.ShopStatus,'') = '' OR ShopMaster.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') <> 'ShopAccess' AND IsActive = 1) )";
        ...
        $documentStatusCondition = " AND ShopMaster.SurveyDate IS NOT NULL AND (ISNULL(ShopMaster.ShopOutsideImage1,'') <> '' OR ISNULL(ShopMaster.ShopOutsideImage2,'') <> '') AND ( ISNULL(ShopMaster.ShopStatus,'') = '' OR ShopMaster.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') <> 'ShopAccess' AND ShopMaster.ShopStatus <> 'Verified' AND IsActive = 1) ) ";
        // $doc_InReviewOrRejectedCondition = " WHERE t1.Shop_Cd IN (SELECT DISTINCT(Shop_Cd) FROM ShopDocuments WHERE ShopDocuments.Shop_Cd = t1.Shop_Cd AND ShopDocuments.IsActive = 1 ) ";
        $doc_InReviewOrRejectedCondition = " ";
        $doc_InReviewOrRejectedColumnValue = " CASE WHEN (SELECT DISTINCT(Shop_Cd) FROM ShopDocuments WHERE ShopDocuments.Shop_Cd = t1.Shop_Cd AND ShopDocuments.IsActive = 1 ) <> 0 THEN 1 ELSE 0 END AS DocFlag,   ";
    ...
    }else if($documentStatus == "DocDenied"){
        $documentStatusCondition = " AND ShopMaster.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') = 'ShopAccess' AND IsActive = 1 ) ";
        $doc_InReviewOrRejectedCondition = " ";
        $doc_InReviewOrRejectedColumnValue = " CASE WHEN (SELECT DISTINCT(Shop_Cd) FROM ShopDocuments WHERE ShopDocuments.Shop_Cd = t1.Shop_Cd AND ShopDocuments.IsActive = 1 ) <> 0 THEN 1 ELSE 0 END AS DocFlag,   ";
        $documentStatusName = "Documents Denied";
    }else{
        $documentStatusCondition = "  ";
        $doc_InReviewOrRejectedCondition = " ";
        $doc_InReviewOrRejectedColumnValue = " CASE WHEN (SELECT DISTINCT(Shop_Cd) FROM ShopDocuments WHERE ShopDocuments.Shop_Cd = t1.Shop_Cd AND ShopDocuments.IsActive = 1 ) <> 0 THEN 1 ELSE 0 END AS DocFlag,   ";
    ...
    if($documentStatus == "DocPending"){
        $displayTotalCount = 1134;
    }else if($documentStatus == "DocInReview"){
        $displayTotalCount = $total_count["FilteredShop"]+32;
    }else if($documentStatus == "DocReceived"){
        $displayTotalCount = $total_count["FilteredShop"]+31;
    }else{
        $displayTotalCount = $total_count["FilteredShop"];
    }
?>
