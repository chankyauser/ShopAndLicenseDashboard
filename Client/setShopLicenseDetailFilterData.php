<?php
    ...
    if(!isset($_SESSION['SAL_License_Status'])){
     ...
     if(!isset($_SESSION['SAL_Transaction_Status'])){
        $_SESSION['SAL_Transaction_Status'] = "All";
        $transactionStatus = $_SESSION['SAL_Transaction_Status'];
    }else{
        $transactionStatus = $_SESSION['SAL_Transaction_Status'];
    }
    ...
    if($licenseStatus == "All"){
        $documentStatusCondition = " AND ShopMaster.SurveyDate IS NOT NULL AND ( ShopMaster.ShopStatus = 'Verified' OR ShopMaster.ShopApproval = 'Verified'  ) ";
    ...
    if($transactionStatus=="All"){
        ...
        $transactionStatusCondition = " WHERE t2.TransStatus = 'Done' ";
    }else if($transactionStatus=="Unpaid"){
        $transactionStatusCondition = " WHERE ( t2.TransStatus <> 'Done' OR t2.TransStatus = '' ) ";
    }
        ...
        (   
            ...
            SELECT t1.Shop_Cd,
                ISNULL(( SELECT
                    ...
                    FROM ShopBilling sb 
                    INNER JOIN ShopMaster sm on ( sm.Shop_Cd = sb.Shop_Cd AND sb.Shop_Cd = t1.Shop_Cd 
                    ...
                    WHERE sb.Transaction_Cd is not null AND tsd.Shop_Cd = t1.Shop_Cd 
            ...
            <div class="col-lg-8 col-md-7 col-12 col-sm-12">
                <div class="breadcrumb">
                    <a href="index.php" rel="nofollow"><i class="fi-rs-home mr-5"></i>Home</a>
                        <span></span> <a href="#" class="inactiveLink " >  <i class="fi-rs-location-alt"></i> Ward : </a> <?php if($nodeName!="All"){ echo $nodeName." - "; } ?>  <?php if($nodeCd != "All"){ if(sizeof($nodeData)>0){ echo $nodeData["Ward_No"]." - ".$nodeData["Area"]; } }else{ echo $nodeCd; } ?> <span></span> <a href="#" class="inactiveLink" > <i class="fi-rs-document"></i> Shop License Status : </a> <?php echo $licenseStatusName; ?> <span> Shops :  <strong class="text-brand" style="font-size: 17px;font-weight: bold;"><?php echo $total_count["FilteredShop"]; ?> found!</strong> </span>
                </div>
            </div>
                ...
                <div class="pagination-area" style="float:right;">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-start">
                            ...
                            ?>  
                                <li class="page-item"><a class="page-link" onclick="setShopLicenseDetailFilter(<?php  if($loopStart==1){ echo "1"; }else{ echo ($loopStart - 1); } ?>)" ><i class="fi-rs-arrow-small-left"></i></a></li>
                            <?php } ?>
                            ...
                            <?php
                                for($i=$loopStart;$i<=$loopStop;$i++){ 
                                    ...
                                    ?>
                                    <li class="page-item <?php echo $activePageCondition; ?>"><a class="page-link" onclick="setShopLicenseDetailFilter(<?php echo $i; ?>)" ><?php echo $i; ?></a></li>
                            <?php } ?>
                            <?php if($totalRecords > $loopStop){ ?> 
                                <li class="page-item"><a class="page-link"  onclick="setShopLicenseDetailFilter(<?php echo ($loopStop + 1);  ?>)" ><i class="fi-rs-arrow-small-right"></i></a></li>
                            <?php }  ?>
