...
$developmentMode=$_SESSION['SAL_DevelopmentMode'];
    ...
    if($Condition == 'ExecutiveWise')
    {
        ...
        $executiveFilterCondition = " AND ShopMaster.SRExecutive_Cd = $ConditionName ";
        $wardFilterCondition = " ";
...
<?php
    ...
    if($FilterName == 'DocumentPending')
    {
        ...
        AND PocketMaster.IsActive = 1 ) 
        INNER JOIN NodeMaster ON PocketMaster.Node_Cd = NodeMaster.Node_Cd 
	    LEFT JOIN ShopDocuments ON ShopDocuments.Shop_Cd = ShopMaster.Shop_Cd
        WHERE ShopMaster.IsActive=1 AND SurveyDate IS NOT NULL
		ANd ShopDocuments.Shop_Cd IS NULL
        $dateCondition 
        $pcktCondition  
        $supervisorCondition  
        $nodeCondition
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="btn" style="background-color:#c90d41;color:white;font-weight:bold;"> <?php echo $FilterType." - ".$FilterName." - ".$total_count["ShopListingCount"]; ?></span> <b style="font-weight:bold;font-size:16px;"><?php if($Condition == 'ExecutiveWise') { echo " - Shop Survey by - ".$exeNameData['ExecutiveName']; } elseif($Condition == 'WardWise') { echo " - Shop Survey Ward No. ".$ConditionName; }?></b>
</div>
