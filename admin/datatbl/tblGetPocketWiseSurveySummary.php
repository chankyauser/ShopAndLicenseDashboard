...
if(isset($_GET['nodeCd'])){
    ...
    if($nodeCd == 'All'){
        ...
        $nCondition = " AND pmm.Node_Cd <> 0  "; 
    }else{
        $nodeCondition = " AND pm.Node_Cd = $nodeCd AND pm.Node_Cd <> 0  "; 
        $nCondition = " AND pmm.Node_Cd = $nodeCd AND pmm.Node_Cd <> 0  "; 
    }

    if($nodeName == 'All'){
        $nodeNameCondition = " AND nm.NodeName <> '' ";
        $nNCondition = " AND nmm.NodeName <> '' ";
    }else{
        $nodeNameCondition = " AND nm.NodeName = '$nodeName' ";
        $nNCondition = " AND nmm.NodeName = '$nodeName' ";
    }
    ...
    $query = "SELECT COALESCE(a.Pocket_Cd, 0) as Pocket_Cd, COALESCE(pmm.Node_Cd, 0) as Node_Cd, 
                ...
                ISNULL((SELECT COUNT(sm.Shop_Cd) FROM ShopMaster sm WHERE sm.Pocket_Cd = a.Pocket_Cd AND sm.IsActive=1 AND sm.SurveyDate IS NOT NULL AND ( ISNULL(sm.ShopStatus,'') = '' OR sm.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') <> 'ShopAccess' AND IsActive = 1))),0) as SurveyAll, 


    ISNULL((SELECT COUNT(sm.Shop_Cd) FROM ShopMaster sm INNER JOIN PocketMaster pm ON ( sm.Pocket_Cd = pm.Pocket_Cd AND pm.IsActive = 1 ) INNER JOIN NodeMaster nm ON pm.Node_Cd = nm.Node_Cd WHERE sm.IsActive=1 AND sm.SurveyDate IS NULL AND sm.Pocket_Cd = a.Pocket_Cd AND ( ISNULL(sm.ShopStatus,'') = '' OR sm.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') <> 'ShopAccess' AND IsActive = 1) ) $nodeCondition $nodeNameCondition  ),0) as SurveyPending,
    ISNULL((SELECT COUNT(sm.Shop_Cd) FROM ShopMaster sm INNER JOIN PocketMaster pm ON ( sm.Pocket_Cd = pm.Pocket_Cd AND pm.IsActive = 1 ) INNER JOIN NodeMaster nm ON pm.Node_Cd = nm.Node_Cd WHERE sm.IsActive=1 AND sm.Pocket_Cd = a.Pocket_Cd AND sm.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') = 'ShopAccess' AND IsActive = 1 AND ShopStatus = 'Permanently Closed') $nodeCondition $nodeNameCondition  ),0) as PC,
    ISNULL((SELECT COUNT(sm.Shop_Cd) FROM ShopMaster sm INNER JOIN PocketMaster pm ON ( sm.Pocket_Cd = pm.Pocket_Cd AND pm.IsActive = 1 ) INNER JOIN NodeMaster nm ON pm.Node_Cd = nm.Node_Cd WHERE sm.IsActive=1 AND sm.Pocket_Cd = a.Pocket_Cd AND sm.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') = 'ShopAccess' AND IsActive = 1 AND ShopStatus = 'Permission Denied') $nodeCondition $nodeNameCondition  ),0) as PD,
    ISNULL((SELECT COUNT(sm.Shop_Cd) FROM ShopMaster sm INNER JOIN PocketMaster pm ON ( sm.Pocket_Cd = pm.Pocket_Cd AND pm.IsActive = 1 ) INNER JOIN NodeMaster nm ON pm.Node_Cd = nm.Node_Cd WHERE sm.IsActive=1 AND sm.Pocket_Cd = a.Pocket_Cd AND sm.ShopStatus in (SELECT ApplicationStatus as ShopStatus FROM StatusMaster WHERE ISNULL(Remark,'') = 'ShopAccess' AND IsActive = 1 AND ShopStatus = 'Non-Cooperative') $nodeCondition $nodeNameCondition  ),0) as NC,
                            ...
                            <!-- <table class="table row-grouping"> -->
                                <thead>
                                    <tr>
                                        ...
                                        <th colspan="7">Survey OverAll</th>
                                    </tr>
                                    <tr>
                                        <th>SrNo</th>
                                        <th>Pocket </th>
                                    ...
                                    <?php
                                        ...
                                        foreach ($dataPocketMapAndListSummary as $key => $value) {
                                            $srNo = $srNo + 1;
                                             ...
                                             $totalShopsPC = $totalShopsPC + $value["PC"];
                                             $totalShopsPD = $totalShopsPD + $value["PD"];
                                             $totalShopsNC = $totalShopsNC + $value["NC"];
                                             $totalShopsPending = $totalShopsPending + $value["SurveyPending"];
                                   ?> 
                                        <tr>
                                            <td><?php echo $srNo; ?></td>
                                            ...
                                            <td><?php echo ($value["SurveyPending"]); ?></td>
                                            <td>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <th><?php echo $totalShopsPC; ?></th>
                                        <th><?php echo $totalShopsPD; ?></th>
                                        <th><?php echo $totalShopsNC; ?></th>
