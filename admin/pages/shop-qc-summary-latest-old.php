...
<div class="tab-content">
    <div class="tab-pane active " id="qc-summary-over-all" aria-labelledby="qc-summary-over-all-tab" role="tabpanel">
        ...
        <?php
            ...
            (SELECT ISNULL(COUNT(sm.Shop_Cd),0)
            ...
            WHERE CONVERT(VARCHAR,sm.AddedDate,120) BETWEEN '$fromDate' AND '$toDate' AND sm.SurveyDate IS NOT NULL
            AND sm.IsActive = 1
            AND pm.IsActive = 1
            AND nm.IsActive = 1
            AND (sm.ShopKeeperMobile IS NULL OR LEFT(sm.ShopKeeperMobile, 1) NOT IN (6,7,8,9) OR LEN(sm.ShopKeeperMobile) != 10 ) ) as MobilePending,
            (SELECT ISNULL(COUNT(sm.Shop_Cd),0)
            FROM ShopMaster as sm
            INNER JOIN PocketMaster pm on pm.Pocket_Cd = sm.Pocket_Cd
            INNER JOIN NodeMaster nm on nm.Node_Cd = pm.Node_Cd
            ...
            AND pm.IsActive = 1
            AND nm.IsActive = 1
            AND ( sm.ShopOutsideImage1 IS NULL AND sm.ShopOutsideImage2 IS NULL ) ) as PhotoPending,
            (SELECT ISNULL(COUNT(sm.Shop_Cd),0)
            FROM ShopMaster as sm
            INNER JOIN PocketMaster pm on pm.Pocket_Cd = sm.Pocket_Cd
            INNER JOIN NodeMaster nm on nm.Node_Cd = pm.Node_Cd
            ...
            AND pm.IsActive = 1
            AND nm.IsActive = 1
            AND sd.Shop_Cd IS NULL AND sm.ShopStatus = 'Pending' ) as DocumentPending,
            (SELECT ISNULL(COUNT(sm.Shop_Cd),0)
            FROM ShopMaster as sm
            INNER JOIN PocketMaster pm on pm.Pocket_Cd = sm.Pocket_Cd
            INNER JOIN NodeMaster nm on nm.Node_Cd = pm.Node_Cd
            WHERE CONVERT(VARCHAR,sm.AddedDate,120) BETWEEN '$fromDate' AND '$toDate' AND sm.SurveyDate IS NULL
            AND sm.IsActive = 1
            AND pm.IsActive = 1
            AND nm.IsActive = 1
            AND sm.ShopStatus = 'Permanently Closed' ) as PermanantlyClosed,
            (SELECT ISNULL(COUNT(sm.Shop_Cd),0)
            FROM ShopMaster as sm
            INNER JOIN PocketMaster pm on pm.Pocket_Cd = sm.Pocket_Cd
            INNER JOIN NodeMaster nm on nm.Node_Cd = pm.Node_Cd
            ...
            AND pm.IsActive = 1
            AND nm.IsActive = 1
            AND sm.ShopStatus = 'Non-Cooperative' ) as NonCooperative,
            (SELECT ISNULL(COUNT(sm.Shop_Cd),0)
            FROM ShopMaster as sm
            INNER JOIN PocketMaster pm on pm.Pocket_Cd = sm.Pocket_Cd
            INNER JOIN NodeMaster nm on nm.Node_Cd = pm.Node_Cd
            ...
            AND pm.IsActive = 1
            AND nm.IsActive = 1
            AND sm.ShopStatus = 'Permission Denied' ) as PermissionDenied
            FROM ShopMaster as sm
            INNER JOIN PocketMaster pm on pm.Pocket_Cd = sm.Pocket_Cd
            INNER JOIN NodeMaster nm on nm.Node_Cd = pm.Node_Cd
...
<div class="row">
        ...
        <div class="col-lg-2 col-sm-6 col-12">
            <div class="card">
                <div class="card-header d-flex align-items-start pb-0">
                    <div>
                        <h2 class="text-bold-700 mb-0"><?php echo IND_money_format($DataValidationData["DocumentPending"]); ?></h2>
