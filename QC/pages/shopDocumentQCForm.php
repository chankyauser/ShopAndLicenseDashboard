...
<?php 
    ...
    $ST_Date = "";
    // $queryST = "SELECT CONVERT(VARCHAR,ST_DateTime,23) as ST_Date, (SELECT COALESCE(em.ExecutiveName, '') FROM Survey_Entry_Data..Executive_Master em WHERE em.Executive_Cd=ShopTracking.ST_Exec_Cd) as ST_ExecutiveName  from ShopTracking Where ScheduleCall_Cd = $DocScheduleCall_Cd AND ST_Status = 1";
    $queryST = "SELECT CONVERT(VARCHAR,st.ST_DateTime,23) as ST_Date, ISNULL(sd.CallReason,'') as ScheduleReason, ISNULL(sd.Remark,'') as ScheduleRemark,  (SELECT COALESCE(em.ExecutiveName, '') FROM Survey_Entry_Data..Executive_Master em WHERE em.Executive_Cd=st.ST_Exec_Cd) as ST_ExecutiveName  from ShopTracking st INNER JOIN ScheduleDetails sd on sd.ScheduleCall_Cd = st.ScheduleCall_Cd Where st.ScheduleCall_Cd = $DocScheduleCall_Cd AND st.ST_Status = 1";
    ...
    if(sizeof($STForQC)>0){
        ...
        $getScheduleReason = $STForQC["ScheduleReason"];
        $getScheduleRemark = $STForQC["ScheduleRemark"];
    }

    $ScheduleCall_Cd = 0;

    
    $queryScheduleDoc = "SELECT st.ScheduleCall_Cd from ShopTracking st Where st.ST_Status = 1 AND CONVERT(VARCHAR,st.ST_DateTime,23) = '$ST_Date' AND st.Calling_Category_Cd = (SELECT TOP 1 Calling_Category_Cd  FROM CallingCategoryMaster WHERE Calling_Type = 'Survey' AND QC_Type = 'ShopSurvey' ) AND st.Shop_Cd = $Shop_Cd ";
    $ScheduleSurveyForQC = $db->ExecutveQuerySingleRowSALData($queryScheduleDoc, $electionName, $developmentMode);

    if(sizeof($ScheduleSurveyForQC)>0){
        $ScheduleCall_Cd = $ScheduleSurveyForQC["ScheduleCall_Cd"];
    }else{
        $queryScheduleDoc = "SELECT st.ScheduleCall_Cd from ShopTracking st Where st.ST_Status = 1 AND st.Calling_Category_Cd = (SELECT TOP 1 Calling_Category_Cd  FROM CallingCategoryMaster WHERE Calling_Type = 'Survey' AND QC_Type = 'ShopSurvey' ) AND st.Shop_Cd = $Shop_Cd ORDER BY st.ST_DateTime DESC";
        $ScheduleSurveyForQC = $db->ExecutveQuerySingleRowSALData($queryScheduleDoc, $electionName, $developmentMode);
        if(sizeof($ScheduleSurveyForQC)>0){
            $ScheduleCall_Cd = $ScheduleSurveyForQC["ScheduleCall_Cd"];
        }
...
<div class="col-12 col-sm-12">
    <fieldset>
        <legend><b><?php echo $srNo.") "; ?> Shop Document QC  :: <?php echo $getShopName." - ".$getCalling_Category." - ".$getScheduleReason." - ".$getScheduleRemark." - ".$getSurveyByName; ?>  - <?php echo date('d/m/Y',strtotime($ST_Date)); ?></b></legend>
        ...
        <div class="col-12 col-sm-12 col-md-12" style="margin-top: 10px;" >
            ...
            <?php } ?>
                ...
                <div class="tab-pane" id="document-qc-info-<?php echo $DocScheduleCall_Cd; ?>" aria-labelledby="document-qc-info-<?php echo $DocScheduleCall_Cd; ?>-tab" role="tabpanel">
                     <?php  include 'shopDocumentsListQCForm.php' ?>
                </div>
                            ...
                            <label>Application Status</label>
                                ...
                                <select class="select2 form-control" name="<?php echo $ScheduleCall_Cd; ?>_ShopStatus">
                                    <option  value="">--Select--</option>   
                                        ...
                                        if (sizeof($ShopStatusDropDown)>0) 
                                            {
