...
<?php 
    ...
    $queryST = "SELECT CONVERT(VARCHAR,st.ST_DateTime,23) as ST_Date, ISNULL(sd.CallReason,'') as ScheduleReason, ISNULL(sd.Remark,'') as ScheduleRemark,  (SELECT COALESCE(em.ExecutiveName, '') FROM Survey_Entry_Data..Executive_Master em WHERE em.Executive_Cd=st.ST_Exec_Cd) as ST_ExecutiveName  from ShopTracking st INNER JOIN ScheduleDetails sd on sd.ScheduleCall_Cd = st.ScheduleCall_Cd Where st.ScheduleCall_Cd = $ScheduleCall_Cd AND st.ST_Status = 1";
...
<div class="col-12 col-sm-12">
    <fieldset>
        <legend><b><?php echo $srNo.") "; ?> Shop QC :: <?php echo $getShopName." - ".$getCalling_Category." - ".$getScheduleReason." - ".$getScheduleRemark." - ".$getSurveyByName; ?>  - <?php echo date('d/m/Y',strtotime($ST_Date)); ?></b></legend>
